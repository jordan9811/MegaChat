...
| B4 | Real code checker | `CodeChecker` → `OcrCodeChecker` | Open question: OCR the whole frame (robust, slow) or crop to the badge's expected position (fast, but breaks when a streamer repositions the source — which they're allowed to do). |
...
- **G1 — The watermark proves the overlay was on screen, not that MegaChats played.** See the design critique in `HANDOFF-BOUNTY.md`. This is the most significant gap in the mechanic as specified, and it is not something Run B can paper over — it needs a product decision.
- **G2 — Badge size check cannot see OBS scene transforms.** A page can't observe its own scene scaling, so the client check catches a tiny browser-source resolution but not a source scaled down in the scene. Enforcement therefore lives in the verifier (unreadable badge ⇒ failed check ⇒ no payout), and the client check is an early-warning affordance only. The spec's framing that "detection IS the payout trigger" is only true of verification-time detection.
- **G3 — Badge violations are client self-reported.** Safe only because the incentive aligns: a client that stays silent still fails verification. Reporting exists to make the failure legible to the streamer, not to secure anything.
...
- **G6 — AMBIGUOUS has no review queue.** It's a distinct verifier result and surfaces in admin with a `review` marker, but nothing routes it to a human automatically.
- **G7 — Ledger is not append-only at the filesystem level.** It's append-only by construction (no update/delete writer exists), but `bounty.json` is rewritten wholesale on each save, so a corrupted write could still lose history. Real durability wants an append-only log or a DB.
- **G8 — `feat/bounty-claim-runA` branched from `v0-ui-migration`, which does NOT contain the LiveKit lazy-connect work** (that's unmerged on `fix/livekit-lazy-connect`). Both branches modify `public/overlay.html` in different regions; expect a small merge, not a conflict-free fast-forward. The bounty badge is deliberately isolated at the end of the script and outside `#stage` to keep that merge trivial.
