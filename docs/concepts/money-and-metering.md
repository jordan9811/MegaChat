...
**Which of these a viewer actually gets:** the join page calls `/api/join/passkey` for both wallet kinds it supports — Privy embedded wallets and MetaMask (`web/lib/join-page.ts`, header: "ONE metered join flow for both modes against /api/join/passkey"). The MPP route is mounted and gated, but nothing in `web/lib/join-page.ts` calls it. The older Gateway prepaid route `POST /api/join` answers `501` ("retired in the Tempo migration", `server.js`). Whether `/api/join/mpp` is a live alternative or retained code is [U1 on the status-call list](../internal/needs-a-status-call.md).
