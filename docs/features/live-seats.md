...
**The streamer's side.** In LiveKit rooms the streamer's own return camera arms once and publishes itself whenever a guest goes live, so a guest sees the host without the host pressing anything per guest (`web/components/host-cam-card.tsx`, header). If the host's camera is held by another application, the booth keeps retrying every 6 s and shows an alert saying guests can hear but not see them (`web/components/host-cam-card.tsx`, `CAM_RETRY_MS`; `_gate-cohost-booth.mjs`). The host's picture is shown — in the booth's own preview and on the guest's page — only once the booth has confirmed it is moving. Until then, and whenever it is OBS's own "virtual camera not started" placeholder or a frozen frame, both sides show a designed state instead and the host's voice carries on; the booth tells guests which through a LiveKit participant attribute, `mc.picture` (`web/components/host-cam-card.tsx`, `LIVE_AFTER`, `OBS_PLACEHOLDER`; `web/lib/join-page.ts`, `mountLivekitHostFeed`; `_gate-booth-picture.mjs`). The picture returns by itself when it moves again: clicking Start Virtual Camera in OBS needs no re-arm. Where the streamer hears guests is a three-way choice on the booth card. "This tab", the default, plays the guests in the booth, where Chrome's echo canceller removes them from the streamer's mic, and the overlay mutes exactly those guests, so each reaches the stream once through OBS Desktop Audio. "OBS · headphones" leaves the guests to the overlay, as before, for a streamer on headphones. "OBS · speakers" does the same but asks Chrome for echoCancellation:'all', which cancels everything the PC plays — at the cost of about 170 ms on the streamer's voice, and of clipping it for guests while the PC plays something loud; if Chrome will not grant 'all', the booth falls back to This tab (`web/components/host-cam-card.tsx`, `AecMode`; `public/overlay.html`, `applyGuestAudio`; `_gate-booth-audio.mjs`; `_gate-overlay-first-guest.mjs`; `_probe-aec-loopback.mjs` and `_probe-aec-doubletalk.mjs` measure the canceller on a given machine). The moment anyone takes a seat, the seat table, the MegaChat queue and the booth move to the top of the manage page's right column (`web/components/create-room/create-room.tsx`, `liveFirst`).
