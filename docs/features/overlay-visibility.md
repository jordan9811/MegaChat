...
- **It does not gate a payout on its own.** Pass C Part 3 consumes the signals: `overlay_hidden` banks a pledged clip for replay and pauses the seat meter, `overlay_visible` drains the bank and sweeps seat money, and `overlay_scaled_below_floor` is a named review cause ([Bounty program](bounty-program.md), [Live seats](live-seats.md)). Every one of those is accounting with stub settlement, and broadcast capture remains the payout authority.
