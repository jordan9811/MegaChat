...
- **Kick and VODs.** The notice shown to Kick streamers says "Kick has no VOD we can read". `OPEN-ISSUES.md` (2026-08-29, "Terminology correction") records that Kick *does* keep VODs; what it lacks is a sanctioned VOD *discovery* API, which is the thing the verifier needs. The copy was not changed after the correction. Which wording is intended is [U2](../internal/needs-a-status-call.md).
