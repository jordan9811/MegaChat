...
| E37 | **No path puts an approved pledged clip on air the first time.** `bounty-clips.markPlayed` is called by nothing and `playCount` is 0 for every stored clip that has existed; playback windows open only through the letters hook and the admin rehearsal route. Pass C Part 3 added the bank's replay bridge (`letters.js`, `enqueueStoredClip`) — the first mechanism that plays a stored clip — but only for replays. The initial play needs a route and a control on the streamer's queue. This blocks the bounty mechanic as a product. | `OPEN-ISSUES.md`, 2026-09-16 Session 2 |
| E38 | **The seat pending bucket is not yet real money.** Ticks still pull straight to the payout address (`server.js`, `tickPasskeyStreamSeat`); `seat-escrow.js` records what should happen and emits stub intents. Making it real = redirect ticks to a platform-held balance + `RealSettlement` against those intents, with a funded wallet — the same work as row 24 of the retest checklist. | `seat-escrow.js`, header; `DECISIONS.md`, 2026-09-16 |
| E39 | **MPP seats do not pause while the overlay is hidden.** Their ticks are client vouchers; refusing one trips the stale-kick and ends the seat. Only the server-driven meters skip ticks. | `server.js`, `tickAllMeters`; register L34 |
