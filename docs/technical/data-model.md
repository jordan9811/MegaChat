...
- **Platform OAuth tokens or scopes beyond identity.** The direct Twitch path requests no scopes and reads identity with an app token (`auth.js`, `scope: ''`); Privy holds the social session, not MegaChat (`privy-identity.js`). An access token lives only for the length of the callback that fetches a link's attributes and is never written down — which is why refreshing them is a re-authorisation rather than a background job (`auth.js`, the callback's `grabAttributes`; `accounts.js`).
- **Any social graph.** The account layer stores follower and following COUNTS and nothing else: no follower lists, no following lists, no mutuals, no edges of any kind, at any price. The X adapter makes one profile lookup per link and never pages a list (`attestation/x-api.js`); the attribute shape has no room for a collection of people (`attestation/index.js`, `ATTRIBUTES`).
- **A verification step.** No attribute is ever collected by asking the person for one. Everything a link carries came back from the platform inside the OAuth round trip they were already in, and a fetch that fails leaves `attributes: null` rather than prompting (`auth.js`, the callback's `grabAttributes`).
