...
| 8 | During a bounty air session, the page polls OBS for the overlay's scene-item state (`GetCurrentProgramScene` → `GetSceneItemList` → `GetSceneItemEnabled`/`Transform`) every `obsScenePollMs` and posts each sample to the server. `NO_CONNECTION` on the manual path is a normal outcome. | `web/lib/obs-scene-check.mjs`; `POST /api/bounty/air-session/:id/obs-scene`; `recordObsSceneSample`, `bounty-evidence.js` | — |
