...
import { ChevronRight, Gift, LockKeyhole, MessageSquareText, Video, type LucideIcon } from 'lucide-react'
...
export function CreateRoom() {
  const { draft, updateDraft, create, hasIdentity, identityHandle, saveDefaultsFromDraft, myRooms, openOwnedRoom } = useRoom()
  ...
  return <div className="mc-create">
    <header className="mcc-header"><div className="mcc-header-inner"><span className="mcc-brand-lockup"><a href="/?stay=1" className="mcc-brand"><BrandText /></a><i /><span>New room</span></span><nav aria-label="Product navigation"><a href="/app">Rooms</a><a href="/bounty">Bounties</a><a href="/how-it-works">How it works</a></nav><AccountChip accent="var(--mcc-cyan)" /></div></header>
    <main className="mcc-shell">
      {!hasIdentity ? <RoomRecovery /> : null}
      <section className="mcc-identity" aria-label="Room identity"><div><span className="mcc-eyebrow">Room name</span><InlineText value={draft.name} onChange={(name) => updateDraft({ name })} placeholder="Name your room" label="Room name" /><small>{hasIdentity ? `Suggested from your linked ${draft.twitchChannel ? 'Twitch' : 'streaming account'} · click to change` : 'Suggested name · click to change'}</small></div><div><span className="mcc-eyebrow">Your room link</span><span className="mcc-room-url">megachat.fun/<InlineText className="mcc-handle" value={draft.handle} onChange={(handle) => updateDraft({ handle })} sanitize={(value) => value.toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 20)} placeholder={identityHandle || 'yourname'} label="Your link" /></span><small>{hasIdentity ? 'Reserved by your sign-in · click the name to change' : 'Suggested link · easy to override'}</small></div></section>

      <div className="mcc-layout"><div className="mcc-menus"><section className="mcc-viewer-section"><h1>How viewers join</h1><div className="mcc-feature-stack">
