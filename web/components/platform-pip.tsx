// The little platform badge that rides in the corner of a streamer's avatar.
// Shared by the landing's bounty thumbnails and the rooms board, so a face
// says who and where in one slot on both pages.
...
export function PlatformPip({
  platform,
  // pump.fun's mark is a capsule with a notch cut through it — the notch has
  // to be painted the colour of whatever sits behind the pip, so callers on a
  // different panel pass their own.
  notch = 'var(--mc-pip-notch, #0a0c12)',
}: {
  platform: string | null
  notch?: string
}) {
  const k = String(platform || '').toLowerCase()
  if (k === 'pumpfun' || k === 'pump.fun') {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <g transform="rotate(-45 12 12)">
          <rect x="0.5" y="7.25" width="23" height="9.5" rx="4.75" />
          <rect x="11.3" y="7.25" width="1.4" height="9.5" fill={notch} />
        </g>
      </svg>
    )
  }
